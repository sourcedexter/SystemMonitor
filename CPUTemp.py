...
def getDegree():
	degree = 0
	while degree == 0:
		degree = raw_input("(F)ahrenheit, (C)elsius, (K)elvin: ") 
...
def main():
	...
	exitmsg = ""
	try:
		opts, args = getopt.getopt(sys.argv[1:], "aCFKs:hV", ["average","celsius", "kelvin", "fahrenheit", "seconds=", "help", "version"])
	...
	if avg == 1:
		return 0
	elif os.path.exists("/var/log/cputemp.log") == True and avg == 0 :
		import datetime
